...
const Sidebar = () => {
    const [drawer, setDrawer] = React.useState(false);
      ...
      const list = (anchor) => (
        <Box
          sx={{ width: anchor === 'top' || anchor === 'bottom' ? 'auto' : 250, backgroundColor: "#0C1A2D", height: "100%"}}
          role="presentation"
        ...
        <div className='flex flex-col p-6 bg-main'>
                ...
                <NavLink
                  ...
                  to="/reviews"
                >
                <ChatBubbleOutlineOutlinedIcon/>  <span className="p-3 px-1">Review</span>
                </NavLink>
                <NavLink
                  className="m-1   no-underline hover:bg-slate-600 font-medium rounded-md text-base text-white flex items-center"
                  to="/questions"
  ...
  return (
    <>
           <button className="cursor-pointer m-1  p-1 py-3  sm:hidden text-white no-underline hover:underline font-medium rounded-md md:text-base lg:text-lg text-xs md:p-3 md:m-2 flex items-center" onClick={toggleDrawer}>
                <MenuIcon/>
           </button> 
          <Drawer
            anchor="left"
